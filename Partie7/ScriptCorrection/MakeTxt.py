...
reArgsRes = re.compile(r"test_p.+\.\.\. $\narg :  (.+)$\n(\w{2,5})$\n",
                       flags=re.MULTILINE)
reNote = re.compile(r"Ran (\d) tests? in \d{1,3}\.\d{3}s$\n\n((?:\w{6} \((?:failures=(\d))?,?\s?(?:errors=(\d))?\))|OK)$\n",
                    flags=re.MULTILINE)
...
for folder in dirsDepth1:
    ...
    with open(pathTxt, 'w+') as fileresults:
        testfonctionnement = reTestFonctionnement.findall(content)[0]

        resulteleve = reArgsRes.findall(content)
        Note = reNote.findall(content)[-1]
        ...
        if testfonctionnement == 'ERROR':
            ...
            if Note[1] == 'OK':
                ...
                fileresults.write(
                    "Vous avez passé {} tests sur {} ({})\n\n".format(
                        int(Note[0])-(Fails+Err), Note[0], Note[1]))
